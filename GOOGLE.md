...
- **Model Standard**: Defaults to Google Gemini models (e.g., `gemini-2.5-flash` or `gemini-1.5-flash`) running natively via the **Google Gemini API** (`generativelanguage.googleapis.com`) using JSON mode with exponential backoff retries for reliability. Agent orchestration is implemented via Gemini Function Calling API, which is functionally equivalent to Agent Builder's tool-use framework.
