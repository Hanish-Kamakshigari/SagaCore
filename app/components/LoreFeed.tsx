...
export default function LoreFeed({ lore }: LoreFeedProps) {
  return (
        ...
        <AnimatePresence initial={false}>
          {lore.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center text-zinc-500">
              ...
              <p className="text-xs">No entries in the celestial logs yet.</p>
            </div>
          ) : (
            lore.map((item, index) => (
              <motion.div
              ...
              >
                <div className="absolute left-0 top-0 h-full w-[2px] bg-purple-500/50" />
                <div className="flex justify-between items-start gap-3">
                  <p className="leading-relaxed font-medium">{item}</p>
                  <span className="text-[9px] font-mono uppercase tracking-widest text-purple-400/80 shrink-0 mt-0.5">
                    Logged
                  </span>
                </div>
              </motion.div>
